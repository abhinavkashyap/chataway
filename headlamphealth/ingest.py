...
class InMemoryTextIngestor:
    def __init__(self, texts: list[str], metadatas: list[dict], splitter: TextSplitter):
        """Ingests a list of strings available in memory

        Parameters
        ----------
        texts : list[str]
            In memory text that needs to be embedded.
        metadatas: list[dict]
            This is list of metadata information
        splitter : TextSplitter
            Splitter that splits the text into different
            chunks and stores in the vector store for retrieval
        """
        self.texts = texts
    ...
    def ingest(self) -> list[Document]:
        """Returns a list of documents that can be embedded into a vectore store

        Returns
        -------
        list[Document]
            A list of documents that can be embedded using langchain
        """
